...
const drink = type => cost => ([number, money]) =>
  money - cost >= 0
    ? `Drink maker makes 1 ${type} with ${number || 'no'} sugar${number ? 's' : ''} and ${number ? 'a stick' : 'therefore no stick'}`
...
const dispatch = actions => ([action, ...args]) => actions[action](...args);
